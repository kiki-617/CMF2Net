...
# 基础路径（根据实际情况调整）
base_path = Path(
    '/media/lyq/4dbd4ed9-dd80-4bb0-8276-9178451541d2/MMFAFM_BCS/data/RESULTS_FOLDER/nnUNet/3d_fullres/Task133_CMUexternalValDCET2Reg/MAML3_channelTrainerV2BreastRegions__nnUNetPlansv2.1')
...
# 3. 显示五折统计汇总（使用五折平均和五折标准差）
print("\n==== 五折统计汇总 ====")
for metric in metrics_to_analyze:
    cross_stats = calculate_cross_fold_stats(metrics, metric)
    if cross_stats['valid_folds'] > 0:
        print(f"{metric}:")
        print(f"  总体平均(五折平均): {cross_stats['cross_fold_mean']:.4f}")
        print(f"  总体标准差(五折之间): {cross_stats['cross_fold_std']:.4f}")
        print(f"  基于{cross_stats['valid_folds']}个有效折叠")
    else:
        print(f"{metric}: 无有效数据")
