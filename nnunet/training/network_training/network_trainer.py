...
class NetworkTrainer(object):
    ...
    def run_training(self):
        ...
        self.plot_network_architecture() # 没画 直接 return
        ...
        if not self.was_initialized: # False
            self.initialize(True)
        print(f"max_epoch:{self.max_num_epochs}") # 打印最大训练epoch
        while self.epoch < self.max_num_epochs: # 开始训练 到 指定epoch
            self.print_to_log_file("\nepoch: ", self.epoch)
            epoch_start_time = time()
            train_losses_epoch = []
            ...
            if self.use_progress_bar: # False
                ...
                for _ in tqdm(range(self.num_batches_per_epoch)): # 250
                    l = self.run_iteration(self.tr_gen, True) # batch训练迭代
                    train_losses_epoch.append(l) # 所有的patch loss总和

            self.all_tr_losses.append(np.mean(train_losses_epoch)) # 平均训练 loss
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])
            # 训练时 验证
            with torch.no_grad():
                # validation with train=False
                self.network.eval()
                val_losses = []
                for _ in range(self.num_val_batches_per_epoch): # 50
                    l = self.run_iteration(self.val_gen, False, True)
                    val_losses.append(l)
                self.all_val_losses.append(np.mean(val_losses)) # 平均验证 loss
                self.print_to_log_file("validation loss: %.4f" % self.all_val_losses[-1])
            ...
            if not continue_training: # False
                # allows for early stopping
                break

            self.epoch += 1
            self.print_to_log_file("This epoch took %f s\n" % (epoch_end_time - epoch_start_time))
        # 主要目的是将当前的训练轮次（self.epoch）减 1。注释中提到，如果不这么做，在加载 model_final_checkpoint 时可能会出现问题。
        self.epoch -= 1  # if we don't do this we can get a problem with loading model_final_checkpoint.

        if self.save_final_checkpoint:  # True
            self.save_checkpoint(join(self.output_folder, "model_final_checkpoint.model")) # 保存最后一次epoch的模型
        # now we can delete latest as it will be identical with final
        if isfile(join(self.output_folder, "model_latest.model")): # 存在最新的模型
            os.remove(join(self.output_folder, "model_latest.model")) # 移除
        if isfile(join(self.output_folder, "model_latest.model.pkl")): # 存在最新模型的pkl
            os.remove(join(self.output_folder, "model_latest.model.pkl")) # 移除
    """更新模型优化器的学习率"""
    ...
    """根据设定的条件决定是否保存模型的检查点（checkpoint）"""
    ...
    """更新验证评估标准的移动平均值（Moving Average, MA）"""
    ...
    """根据训练过程中的验证评估标准的移动平均值和训练损失的移动平均值，管理训练的耐心机制，决定是否继续训练。耐心机制是一种提前停止训练的策略，当模型在一定轮数内没有显著提升时，停止训练以避免过拟合"""
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA: # 保存最佳的模型
                self.best_val_eval_criterion_MA = self.val_eval_criterion_MA
    ...
    def on_epoch_end(self):
        ...
        self.plot_progress() # 绘制训练和验证过程中的损失曲线以及验证评估指标曲线，并将绘制好的图保存为 progress.png 文件

        self.maybe_update_lr() # 根据不同的策略更新模型的学习率

        self.maybe_save_checkpoint() # 根据设定的条件决定是否保存模型的检查点

        self.update_eval_criterion_MA() # 更新验证评估标准的移动平均值（Moving Average, MA）

        continue_training = self.manage_patience() # True
        return continue_training
    """更新训练损失的移动平均值（Moving Average, MA）。移动平均值是一种常用的统计方法，用于平滑数据序列，减少随机波动的影响，从而更清晰地观察数据的趋势"""
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if run_online_evaluation:
            self.run_online_evaluation(output, target) # 未实现
