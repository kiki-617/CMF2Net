...
class nnUNetTrainerV2BreastRegions(nnUNetTrainerV2_breast):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
        self.regions = get_breast_regions() # 乳腺肿瘤区域(多肿瘤分割有重叠区域)
        self.regions_class_order = (1,) # 乳腺肿瘤类别
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            self.folder_with_preprocessed_data = join(self.dataset_directory, self.plans['data_identifier'] +
                                                      "_stage%d" % self.stage)
            if training:
                self.dl_tr, self.dl_val, self.dl_ts = self.get_basic_generators() # 数据加载器
                if self.unpack_data:
    ...
    def run_online_evaluation(self, output, target):
        ...
        with torch.no_grad():
            ...
            self.online_eval_foreground_dc.append(list((2 * tp_hard) / (2 * tp_hard + fp_hard + fn_hard + 1e-8))) # [[0.0], [0.0]]
            self.online_eval_tp.append(list(tp_hard)) # [[0.0], [0.0]]
            self.online_eval_fp.append(list(fp_hard)) # [[0.0], [0.0]]
            self.online_eval_fn.append(list(fn_hard)) # [[57.0], [10326.0]]
