...
class nnUNetTrainer_breast(NetworkTrainer):
    ...
    #
    #     val_ids = [image.replace('.nii.gz', '') for image in os.listdir(os.path.join(base_folder, 'labels')) if
    #                image.endswith('.nii.gz')]
    #
    #     # 构建训练集和验证集文件路径列表
    #     def get_file_paths(ids, folder):
    #         return [os.path.join(folder, f"{id_}.nii.gz") for id_ in ids]
    #
    #     val_folder = os.path.join(base_folder, 'labels')
    #
    #     dataset_val = get_file_paths(val_ids, val_folder)
    #
    #     self.dataset_val = load_dataset_each_external_val(dataset_val, self.folder_with_preprocessed_data)
    ...
    def finish_online_evaluation(self):
        ...
        self.print_to_log_file("(interpret this as an estimate for the Dice of the different classes. This is not "
                               "exact.)")
        # 把 run_online_evaluation 计算的指标清0 为了下一次计算
        self.online_eval_foreground_dc = []
        self.online_eval_tp = []
        self.online_eval_fp = []
        self.online_eval_fn = []
