...
"""把 nnUNet_preprocessed 的图像npz -> npy"""
...
# 根据划分好的Tr,Val,Ts 分别加载nnUNet_preprocessed的npz和pkl
def load_dataset_each(tr_folder, val_folder, ts_folder, folder, num_cases_properties_loading_threshold=1000):
    # we don't load the actual data but instead return the filename to the np file.
    print('loading dataset')
    tr_case_identifiers = get_case_identifiers_from_raw_folder_label(tr_folder) # BraTS20_Training_003 ....
...
#----------------train------------------
    ...
    if len(tr_case_identifiers) <= num_cases_properties_loading_threshold: # 如果文件数 在阈值内
        print('loading tr all case properties')
        for i in dataset_tr.keys():
            dataset_tr[i]['properties'] = load_pickle(dataset_tr[i]['properties_file']) # 其他信息
...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        ...
        if isfile(self._data[k]['data_file'][:-4] + ".npy"):
            case_all_data = np.load(self._data[k]['data_file'][:-4] + ".npy", self.memmap_mode) # (5,135,163,129)
        else:
            case_all_data = np.load(self._data[k]['data_file'])['data']
        num_color_channels = case_all_data.shape[0] - 1
        data_shape = (self.batch_size, num_color_channels, *self.patch_size) # (2,4,205,205,205)
        seg_shape = (self.batch_size, num_seg, *self.patch_size) # (2,1,205,205,205)
