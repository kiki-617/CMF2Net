...
class SpatialAttention(nn.Module):
    def __init__(self, kernel_size=5):  # 修改为5
        super(SpatialAttention, self).__init__()

        assert kernel_size in (3, 5), 'kernel size must be 3 or 5'  # 修改为3或5
...
class SwiftAttnGate(nn.Module):
    def __init__(self, F_g, F_l, F_int, num_groups=1):
        ...
        # 将 2D 卷积和 2D 批量归一化替换为 3D 版本，使用 Instance Normalization
